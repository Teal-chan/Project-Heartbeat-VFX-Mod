...
func _get_stage_status_localization_key(stage: HBRichPresence.RichPresenceStage) -> String:
	...
	match stage:
		...
		HBRichPresence.RichPresenceStage.STAGE_MEDIA_PLAYER:  # NEW
			out = "#Status_MediaPlayer"
	return out
...
func _update_rich_presence(rich_presence_data: HBRichPresence):
	...
	if rich_presence_data.current_stage == HBRichPresence.RichPresenceStage.STAGE_MEDIA_PLAYER:
		var media_info := rich_presence_data.current_media_title
		if not rich_presence_data.current_media_artist.is_empty():
			media_info += " - " + rich_presence_data.current_media_artist
		Steamworks.friends.set_rich_presence("song", _cap_to_length(media_info, STEAM_DETAIL_MAX_LENGTH))
