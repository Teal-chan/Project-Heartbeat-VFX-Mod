	...
	for item in inspecting_items:
		...
		while (not common_class in item._inheritance) and _data_class != common_class:
			if inheritance_size - i < 0:
				# Walked past the end of the ancestry array without finding a
				# match. Fall back to the universal base so the load() below
				# doesn't get a stale (and now wrong) class name.
				common_class = "EditorTimelineItem"
				break
...
func get_common_data_class():
	...
	for item in inspecting_items:
		...
		while (not common_data_class in item.data._inheritance) and _data_class != common_data_class:
			if inheritance_size - i < 0:
				# CRITICAL: without this guard, walking past the end of the
				# ancestry array uses Godot's negative-index wraparound and
				# the loop oscillates forever -> editor freeze. This was the
				# bug that crashed mixed Note + Metadata selections.
				common_data_class = "HBTimingPoint"
				break
