...
var verify_icon_ok: Texture2D
var verify_icon_error: Texture2D
...
@onready var open_song_meta_button: Button = get_node("VBoxContainer/Panel2/MarginContainer/VBoxContainer/HBoxContainer/OpenSongMetaButton")
@onready var verify_song_button: Button = get_node("VBoxContainer/Panel2/MarginContainer/VBoxContainer/HBoxContainer/VerifySongButton")
@onready var upload_song_button: Button = get_node("VBoxContainer/Panel2/MarginContainer/VBoxContainer/HBoxContainer/UploadSongButton")
@onready var open_external_button: Button = get_node("VBoxContainer/Panel2/MarginContainer/VBoxContainer/HBoxContainer/HBoxContainer/OpenExternalButton")
@onready var icon_pack_button: Button = get_node("VBoxContainer/Panel2/MarginContainer/VBoxContainer/HBoxContainer/HBoxContainer/IconPackButton")
...
func _notify_editor(msg: String) -> void:
	if message_shower:
		message_shower._show_notification(msg)
	else:
		print(msg)
...
func _ready():
	...
	if is_instance_valid(upload_song_button):
		upload_song_button.pressed.connect(_on_UploadSongButton_pressed)
	
	# Connect Open External button
	if is_instance_valid(open_external_button):
		open_external_button.pressed.connect(_on_OpenExternalButton_pressed)
	
	# Connect Icon Pack button
	if is_instance_valid(icon_pack_button):
		icon_pack_button.pressed.connect(_on_IconPackButton_pressed)


func _set_custom_icons():
	# Open meta icon (same as before)
	var meta_image := Image.new()
	var meta_err := meta_image.load("res://tools/icons/meta.svg")
	if meta_err != OK:
		push_error("Failed to load custom meta icon (%s)" % meta_err)
	else:
		var meta_tex := ImageTexture.create_from_image(meta_image)
		open_song_meta_button.icon = meta_tex

	# Verify icon – just load once and keep as the "OK" texture
	var verify_image := Image.new()
	var verify_err := verify_image.load("res://tools/icons/verify.svg")
	if verify_err != OK:
		push_error("Failed to load verify icon (%s)" % verify_err)
	else:
		var verify_tex := ImageTexture.create_from_image(verify_image)
		verify_icon_ok = verify_tex          # save it in the var
		verify_song_button.icon = verify_tex # show it on the button

	# Upload icon
	var upload_image := Image.new()
	var upload_err := upload_image.load("res://tools/icons/upload.svg")
	if upload_err != OK:
		push_error("Failed to load upload icon (%s)" % upload_err)
	else:
		var upload_tex := ImageTexture.create_from_image(upload_image)
		upload_song_button.icon = upload_tex

	# Open external icon
	var open_ext_image := Image.new()
	var open_ext_err := open_ext_image.load("res://tools/icons/open_ext.svg")
	if open_ext_err != OK:
		push_error("Failed to load open_ext icon (%s)" % open_ext_err)
	else:
		var open_ext_tex := ImageTexture.create_from_image(open_ext_image)
		open_external_button.icon = open_ext_tex

	# Icon pack icon
	var icon_pack_image := Image.new()
	var icon_pack_err := icon_pack_image.load("res://tools/icons/icon_pack.svg")
	if icon_pack_err != OK:
		push_error("Failed to load icon_pack icon (%s)" % icon_pack_err)
	else:
		var icon_pack_tex := ImageTexture.create_from_image(icon_pack_image)
		icon_pack_button.icon = icon_pack_tex

	# Start in "OK" state (non-red)
	_update_verify_icon_state(false)
	
func _update_verify_icon_state(has_issues: bool) -> void:
	if not is_instance_valid(verify_song_button):
		return

	# Make sure the icon itself is set at least once
	if verify_icon_ok and verify_song_button.icon == null:
		verify_song_button.icon = verify_icon_ok

	if has_issues:
		# Red tint when there are fatal / important issues
		verify_song_button.modulate = Color(1.0, 0.35, 0.35, 1.0)
	else:
		# Default color (no issues)
		verify_song_button.modulate = Color(1.0, 1.0, 1.0, 1.0)


func _verification_result_has_issues(result) -> bool:
	if result == null:
		return false
	
	# verify_song() returns a Dictionary<string, Array>
	if not (result is Dictionary):
		push_warning("verify_song() returned unexpected type: %s" % typeof(result))
		return false
	
	for key in result.keys():
		var error_list = result[key]
		if error_list is Array and error_list.size() > 0:
			# At least one error/warning in this category
			return true
...
func _on_OpenSongMetaButton_pressed():
	var song := current_song
	if song == null:
		push_error("HBEditor: No current song loaded in the editor.")
		return
	
	var open_chart_popup = open_chart_popup_dialog
	if open_chart_popup == null:
		push_error("HBEditor: Could not access OpenChartPopupDialog.")
		return
	...
	if meta_editor_dialog == null or meta_editor == null:
		push_error("HBEditor: Song meta editor dialog/editor nodes not found.")
		return
	
	var hidden := song.get_fs_origin() == HBSong.SONG_FS_ORIGIN.BUILT_IN and not HBGame.enable_editor_dev_mode
	hidden = hidden or song.comes_from_ugc() \
		or song is HBPPDSong \
		or song is SongLoaderDSC.HBSongMMPLUS \
		or song is SongLoaderDSC.HBSongDSC
...
func _on_VerifySongButton_pressed() -> void:
	verify_current_song()

func verify_current_song() -> void:
	var song: HBSong = current_song
	if song == null:
		_notify_editor("No song is currently loaded in the editor.")
		return
	
	var popup = open_chart_popup_dialog
	if popup == null:
	...
	var has_issues := _verification_result_has_issues(result)
	print("Verify result has_issues =", has_issues) # optional debug

	_update_verify_icon_state(has_issues)
	
	popup.verify_song_popup.show_song_verification(result, false)
	
	if has_issues:
		_notify_editor("Verification found issues for: %s" % song.get_visible_title())
	else:
		_notify_editor("No issues found for: %s" % song.get_visible_title())

func _on_UploadSongButton_pressed() -> void:
	upload_current_song_to_workshop()


func _on_OpenExternalButton_pressed() -> void:
	open_chart_externally()


func open_chart_externally() -> void:
	if current_song == null:
		_notify_editor("No song is currently loaded.")
		return

	var difficulty: String = current_difficulty
	var chart_path: String = current_song.get_chart_path(difficulty)

	if not FileAccess.file_exists(chart_path):
		_notify_editor("Chart file does not exist: " + chart_path)
		return

	# Convert res:// or user:// to absolute filesystem path
	var fs_path: String = ProjectSettings.globalize_path(chart_path)

	# Build a file:// URI for OS.shell_open
	...
	if result != OK:
		_notify_editor("Failed to open chart externally (error %d)" % result)
	else:
		_notify_editor("Opened chart in external editor")


func _on_IconPackButton_pressed() -> void:
	show_icon_pack_dialog()


func show_icon_pack_dialog() -> void:
	var tree = get_tree()
	if tree == null:
		_notify_editor("Cannot show icon pack dialog: get_tree() returned null")
		return
	
	var root = tree.get_root()
	if root == null:
		_notify_editor("Cannot show icon pack dialog: root is null")
		return
	
	var loader = root.get_node_or_null("ResourcePackLoader")
	if loader == null:
		_notify_editor("ResourcePackLoader not found")
		return
...
func _on_icon_pack_dialog_confirmed(dialog: AcceptDialog, option: OptionButton) -> void:
	...
	if chosen_key == current_key:
		_notify_editor("Same pack selected, nothing to change")
		return
	
	_apply_icon_pack(chosen_key)


func _apply_icon_pack(chosen_key: String) -> void:
	var tree = get_tree()
	if tree == null:
		return
	...
	# 3) If this pack should act as a UI skin, hook it in
	...
	# 4) Refresh timeline items
	_icon_pack_refresh_timeline_items()
	
	# 5) Refresh lane header icons
	_icon_pack_refresh_lane_headers()
	
	# 6) Redraw timeline & minimap
	_icon_pack_refresh_timeline_and_minimap()
...
func _icon_pack_refresh_timeline_items() -> void:
	if not has_method("get_timeline_items"):
		return
...
func _icon_pack_refresh_lane_headers() -> void:
	if timeline == null:
		return
...
func _icon_pack_rebuild_preview_notes() -> void:
	var games: Array = []
	
	# Editor preview instance
	if rhythm_game != null:
		games.append(rhythm_game)
	
	# Playtest popup instance
	if rhythm_game_playtest_popup is Node:
		var rg2 = rhythm_game_playtest_popup.get("rhythm_game")
	...
	var items: Array = []
	if has_method("get_timeline_items"):
		items = get_timeline_items()
...
func upload_current_song_to_workshop() -> void:
	...
	# Same 'hidden' rules as OpenChartPopupDialog.populate_tree()
	var hidden := song.get_fs_origin() == HBSong.SONG_FS_ORIGIN.BUILT_IN and not HBGame.enable_editor_dev_mode
	hidden = hidden or song.comes_from_ugc() \
		or song is HBPPDSong \
		or song is SongLoaderDSC.HBSongMMPLUS \
	...
	if hidden:
		_notify_editor("This song cannot be uploaded to the Workshop (official / external / UGC source).")
		return
	...
	# YouTube URL extra error (copied from original script)
	...
	# Block upload if there are fatal / fatal_ugc errors and dev mode is off
	if verification.has_fatal_error(errors, true) and not HBGame.enable_editor_dev_mode:
		...
		_notify_editor("Opened Workshop upload dialog for: %s" % song.get_visible_title())
