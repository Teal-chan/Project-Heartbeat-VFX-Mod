...
func _gui_input(event):
	if event is InputEventMouseButton:
		...
		# MODIFICATION: Handle Metadata layer
		if ln == "Metadata":
			if event.double_click and event.button_index == MOUSE_BUTTON_LEFT:
				var new_metadata = HBMetadataClass.new()
				new_metadata.time = int(editor.snap_time_to_timeline(_cull_start_time + editor.scale_pixels(get_local_mouse_position().x)))
				new_metadata.key = "metadata"
				
				var found_item_at_same_time = false
				for i in get_timing_points():
					if i.time == new_metadata.time:
						found_item_at_same_time = true
						break
				
				if not found_item_at_same_time:
					var item = new_metadata.get_timeline_item()
					item.data = new_metadata
					editor.user_create_timing_point(self, item)
				
				get_viewport().set_input_as_handled()
		elif ln in HBNoteData.NOTE_TYPE.keys():
			if event.double_click and event.button_index == MOUSE_BUTTON_LEFT:
				var new_note = HBNoteData.new()
				new_note.note_type = HBNoteData.NOTE_TYPE[ln]
				new_note.time = int(editor.snap_time_to_timeline(_cull_start_time + editor.scale_pixels(get_local_mouse_position().x)))
