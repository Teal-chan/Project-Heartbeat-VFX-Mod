...
func start_session(game_info: HBGameInfo, assets: SongAssetLoader.AssetLoadToken = null):
	...
	if game_mode is HBGameMode:
		...
		# Connect game-over menu signals (Restart / Quit)
		if game_ui.has_signal("game_over_restart_requested"):
			game_ui.connect("game_over_restart_requested", Callable(self, "_on_game_over_restart_requested"))
		if game_ui.has_signal("game_over_quit_requested"):
			game_ui.connect("game_over_quit_requested", Callable(self, "_on_game_over_quit_requested"))
	else:
...
func _unhandled_input(event):
	if playing_game_over_animation:
		# Block everything during game over — input is handled by
		# the GameOverInputHandler node (PROCESS_MODE_ALWAYS)
...
func _on_RhythmGame_game_over():
	...
	# Menu buttons appear inside play_game_over() after a short delay.
	# Wait for the player to choose Restart or Quit instead of auto-advancing.


func _on_game_over_restart_requested():
	playing_game_over_animation = false
	vhs_panel.hide()
	vhs_panel.material.set_shader_parameter("desaturation", 0.25)

	# Reset the game layer transform that the game-over tween rotated/moved
	game_ui.game_layer_node.rotation = 0
	game_ui.game_layer_node.position = Vector2.ZERO

	# Mirror the pause menu restart path
	var modifiers = []
	for modifier_id in current_game_info.modifiers:
		var modifier = ModifierLoader.get_modifier_by_id(modifier_id).new() as HBModifier
		modifier.modifier_settings = current_game_info.modifiers[modifier_id]
	...
	rollback_on_resume = false
	game.restart()
	# Restore audio volume — game over zeroed it out
	game.notify_song_volume_settings_changed()
	video_player.set_stream_position((current_game_info.get_song().start_time - game.get_latency_compensation_msec()) / 1000.0)
	set_process(true)
	game.set_process(true)
	game.editing = false
...
func _on_game_over_quit_requested():
	# Continue the original fail flow: TV-off → results
