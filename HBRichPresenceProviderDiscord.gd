...
func _update_rich_presence(rich_presence_data: HBRichPresence):
	...
	match rich_presence_data.current_stage:
		...
		HBRichPresence.RichPresenceStage.STAGE_MEDIA_PLAYER:
			var details_title := rich_presence_data.current_media_title
			if not rich_presence_data.current_media_artist.is_empty():
				details_title += " - " + rich_presence_data.current_media_artist
			if details_title.length() > 128:
				details_title = details_title.substr(0, 127)
			queued_update_data["details"] = details_title
			queued_update_data["large_image_key"] = "default"
