...
# NEW: VFX badge label
@onready var vfx_badge_label: Label = %VFXBadge

var data: HBSteamUGCItem
var item_owner: HBSteamFriend

var has_vfx: bool = false
...
func set_data(_data: HBSteamUGCItem):
	...
	# --- NEW: VFX badge based on metadata ---
	_update_vfx_badge()

func _update_vfx_badge() -> void:
	has_vfx = false
	if not vfx_badge_label:
		return

	vfx_badge_label.visible = false

	var meta_str: String = data.metadata
	if meta_str == "":
		return

	var json := JSON.new()
	var err := json.parse(meta_str)
	if err != OK:
		return
	...
	if typeof(parsed) != TYPE_DICTIONARY:
		return
	if not parsed.has("ph_vfx"):
		return

	var ph_vfx = parsed["ph_vfx"]
	if typeof(ph_vfx) != TYPE_DICTIONARY:
		return

	# enabled defaults to true if missing
	if not ph_vfx.get("enabled", true):
		return

	has_vfx = true
	vfx_badge_label.text = "★ VFX"
	vfx_badge_label.tooltip_text = "Includes custom PH VFX"
	vfx_badge_label.visible = true
