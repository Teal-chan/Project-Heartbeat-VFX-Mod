...
func _on_StartButton_pressed():
	...
	# NEW: stamp PHVFX global context so the modifier can find per-chart JSON
	PHVFXModifier.s_last_song_id = String(current_song.id)
	PHVFXModifier.s_last_difficulty = current_difficulty
	print("PreGameScreen: PHVFX context stamp → song_id=%s, diff=%s" %
		[PHVFXModifier.s_last_song_id, PHVFXModifier.s_last_difficulty])
...
func _on_open_modifier_settings_selected(modifier_id: String):
	print("modifier_id: ", modifier_id)
	print("modifiers dict keys: ", game_info.modifiers.keys())
	print("Settings object: ", game_info.modifiers.get(modifier_id))
...
func draw_leaderboard_legality():
	...
	if HBBackend.can_have_scores_uploaded(song):
		# Check if any modifiers block leaderboard legality
		var has_illegal_modifier := false
		for mod_id in game_info.modifiers:
			var mod_class = ModifierLoader.get_modifier_by_id(mod_id)
			if mod_class:
				if not mod_class.is_leaderboard_legal():
					has_illegal_modifier = true
					break
...
func _on_StartPractice_pressed():
	...
	# NEW: stamp PHVFX context for practice as well
	PHVFXModifier.s_last_song_id = String(current_song.id)
	PHVFXModifier.s_last_difficulty = current_difficulty
	print("PreGameScreen: PHVFX context stamp (practice) → song_id=%s, diff=%s" %
		[PHVFXModifier.s_last_song_id, PHVFXModifier.s_last_difficulty])
