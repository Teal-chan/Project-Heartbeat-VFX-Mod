...
func setup(item, index: int):
	...
	# Access nodes directly since @onready may not have run yet
	var title = $VBoxContainer/TitleLabel
	var artist = $VBoxContainer/ArtistLabel
	var duration = $DurationLabel
	
	title.text = item.get_display_title()
	artist.text = item.get_display_artist()
	
	# Format duration
	if item.duration > 0:
		var mins = int(item.duration) / 60
		var secs = int(item.duration) % 60
		duration.text = "%d:%02d" % [mins, secs]
	else:
		duration.text = "--:--"
