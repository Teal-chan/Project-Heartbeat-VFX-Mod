...
const PHVFX = preload("res://rythm_game/modifiers/ph_vfx/ph_vfx.gd")
...
@onready var vfx_indicator: Control = get_node("%VFXIndicator")
...
func set_has_vfx(enabled: bool) -> void:
	if vfx_indicator != null:
		vfx_indicator.visible = enabled			
...
func _song_update():
	...
	var vfx_flag := PHVFX.has_any_vfx_for_song(song)
	set_has_vfx(PHVFX.has_any_vfx_for_song(song))


	var max_stars = song.get_max_score()
	var sorted_diffs: Array = []
	...
	sorted_diffs.sort_custom(func (a: Array, b: Array) -> bool:
		return a[0] < b[0]
	)
