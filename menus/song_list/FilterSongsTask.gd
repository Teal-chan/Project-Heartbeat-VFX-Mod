...
func sort_and_filter_songs():
	...
	# PH Extend: Cache favorites list for filtering
	var _cached_fav_ids: Array = HBSongListMenu._load_favorites() if sort_filter_settings.filter_mode == "favorites" else []
	var _phvfx_script = preload("res://rythm_game/modifiers/ph_vfx/ph_vfx.gd") if sort_filter_settings.filter_mode == "extend" else null
	for i in range(songs.size()):
		...
		match sort_filter_settings.filter_mode:
			...
			&"extend":
				should_add_song = _phvfx_script.has_any_vfx_for_song(song)
			&"favorites":
				should_add_song = song.id in _cached_fav_ids
			&"all", &"folders":
