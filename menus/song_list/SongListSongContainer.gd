...
# PH Extend: Cached favorite IDs for setting star state on items
...
func _create_song_item(song: HBSong):
	...
	item.favorite_toggled.connect(favorite_toggled.emit)
	# PH Extend: Set favorite state
	if song.id in _favorite_ids:
		item.ready.connect(item.set_favorite.bind(true))
	return item
...
func set_songs(_songs: Array, select_song_id: String = "", select_difficulty=null, force_update=false):
	songs = _songs
	_favorite_ids = HBSongListMenu._load_favorites()
