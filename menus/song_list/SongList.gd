...
var filter_localization = {
	...
	"extend": tr("Extend", &"PH Extend song list filter"),
	"favorites": tr("Favorites", &"Favorites song list filter"),
}
...
# --- PH Extend: Favorites persistence ---
const FAVORITES_PATH := "user://extend_favorites.json"
var favorite_song_ids: Array = []

static func _load_favorites() -> Array:
	if not FileAccess.file_exists(FAVORITES_PATH):
		return []
	var rf := FileAccess.open(FAVORITES_PATH, FileAccess.READ)
...
func _save_favorites() -> void:
	var wf := FileAccess.open(FAVORITES_PATH, FileAccess.WRITE)
	wf.store_string(JSON.stringify(favorite_song_ids, "\t"))
	wf.close()

func is_song_favorite(song: HBSong) -> bool:
	return song.id in favorite_song_ids

func toggle_favorite(song: HBSong) -> bool:
	if song.id in favorite_song_ids:
		favorite_song_ids.erase(song.id)
		_save_favorites()
		return false
	else:
		favorite_song_ids.append(song.id)
		_save_favorites()
		return true
...
func _ready():
	...
	song_container.favorite_toggled.connect(_on_favorite_toggled)
...
func populate_buttons():
	...
	# PH Extend tabs
	filter_types.push_back("extend")
	if favorite_song_ids.size() > 0:
		filter_types.push_back("favorites")
...
# --- PH Extend: Favorite toggle handler ---
func _on_favorite_toggled(song: HBSong):
	var is_now_fav := toggle_favorite(song)
	# Update the button on the currently visible item
	for child in song_container.item_container.get_children():
		if child is HBSongListItem and child.song == song:
			child.set_favorite(is_now_fav)
			break
	# If we're on the favorites tab and just unfavorited, refresh the list
	if not is_now_fav and filter_settings.filter_mode == "favorites":
		force_next_song_update = true
		update_songs()
	# Refresh tabs in case favorites list went from 0 to 1 or 1 to 0
	populate_buttons()

	# 🔧 NEW: restore focus to the song list so input + visuals are sane again
	song_container.grab_focus()
