...
func upload_result(song: HBSong, game_info: HBGameInfo) -> BackendRequestToken:
	...
	# Filter out leaderboard-legal (cosmetic) modifiers from the upload
	# The server doesn't need to know about purely visual modifiers
	print("[HBBackend] upload_result called")
	print("[HBBackend] game_info_dict.modifiers = ", game_info_dict.get("modifiers", {}))
	if game_info_dict.has("modifiers") and typeof(game_info_dict.modifiers) == TYPE_DICTIONARY:
		var filtered_modifiers = {}
		for mod_id in game_info_dict.modifiers:
			var mod_class = ModifierLoader.get_modifier_by_id(mod_id)
			print("[HBBackend]   mod_id=", mod_id, " mod_class=", mod_class)
			if mod_class:
				var is_legal = mod_class.is_leaderboard_legal()
				print("[HBBackend]   is_leaderboard_legal=", is_legal)
				if not is_legal:
					# Only include modifiers that affect gameplay
					filtered_modifiers[mod_id] = game_info_dict.modifiers[mod_id]
		print("[HBBackend] filtered_modifiers = ", filtered_modifiers)
		game_info_dict.modifiers = filtered_modifiers
	...
	if song is HBPPDSong:
		...
		if song.guid == "":
			...
			print("[HBBackend] ABORT: PPD song without GUID")
			return
		request.song_ugc_id = song.guid
	else:
		if song.ugc_service_name != "Steam Workshop":
			Log.log(self, "Only UGC songs can have scores uploaded")
			print("[HBBackend] ABORT: Not a Steam Workshop song, ugc_service_name=", song.ugc_service_name)
			return
		request.song_ugc_id = str(song.ugc_id)
	var chart_hash = song.generate_chart_hash(game_info.difficulty)
	print("[HBBackend] chart_hash=", chart_hash, " song_ugc_id=", request.song_ugc_id)
	...
	print("[HBBackend] Making request to /api/leaderboard/enter-result")
