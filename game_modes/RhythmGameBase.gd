...
signal editor_note_judged(judgement)  # For editor hooks - always fires
...
func _on_notes_judged_new(final_judgement: int, judgements: Array, judgement_target_time: int, wrong: bool):
	...
	# Direct editor hook callback (bypasses signal system)
	if has_meta("editor_note_hook"):
		var hook = get_meta("editor_note_hook")
		print("[RHYTHMGAMEBASE] Found hook on instance " + str(get_instance_id()))
		if hook != null and hook.has_method("on_note_judged"):
			hook.on_note_judged(judgement_info)
	else:
		# Only print once to avoid spam
		if not has_meta("_hook_check_printed"):
			print("[RHYTHMGAMEBASE] No hook on instance " + str(get_instance_id()))
			set_meta("_hook_check_printed", true)
	
	emit_signal("editor_note_judged", judgement_info)
