...
func add_new_modifier(modifier_id: String):
	...
	if modifier_class:
		var settings_class = modifier_class.get_modifier_settings_class()
		var settings_instance = settings_class.new()
		modifiers[modifier_id] = settings_instance
		
# Returns if the song is legal for uploading to leaderboards
# keep in mind this doesn't take into account if cheats were used
# that should be checked separately
func is_leaderboard_legal():
	# Check each modifier - if any disallows leaderboards, the whole session is illegal
	for mod_id in modifiers:
		var mod_class = ModifierLoader.get_modifier_by_id(mod_id)
		if mod_class:
			# Call the static method directly - all modifiers should have this
			# since HBModifier base class defines it
			if not mod_class.is_leaderboard_legal():
				return false
	return not result.get_result_rating() == HBResult.RESULT_RATING.FAIL
