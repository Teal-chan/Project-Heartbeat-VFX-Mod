...
## Total score (includes bonus scores)
var score := 0
## Score from holds
var hold_bonus := 0
## Score from chain slides
var slide_bonus := 0
## Score from rush notes
var rush_bonus := 0
var rush_hits := 0
var max_combo := 0
var total_notes := 0
...
func _init():
	...
	"hold_bonus", "slide_bonus", "rush_bonus", "rush_hits", "used_cheats", "percentage_penalty"]

func get_serialized_type():
	return "Result"

func add_percentage_penalty(amount: float) -> void:
	percentage_penalty = clamp(percentage_penalty + amount, 0.0, 0.30)
...
func get_percentage() -> float:
	return get_capped_score() / float(max_score) - percentage_penalty
